...
A PyTorch implementation of the **Deep Compression** pipeline applied to a Multi-Layer Perceptron (MLP) trained on the MNIST dataset. 
This project demonstrates the full compression workflow — pruning, quantization, and Huffman encoding — achieving a **31.3× reduction** in model size with minimal accuracy loss.

---

## Overview

Deep Compression is a model compression technique. It combines three sequential stages to dramatically reduce neural network storage requirements:

1. **Pruning** — Removes redundant weights via magnitude-based thresholding
